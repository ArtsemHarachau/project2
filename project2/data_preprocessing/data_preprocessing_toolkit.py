...
#         df = df[df.is_company == 0]
        return df.loc[df['is_company'].isin([0])]
    ...
    def add_length_of_stay(df):
        ...
        # Write your code here #
        ########################
#         zakomentowana linia powoduje SettingWithCopyWarning
#         df['length_of_stay'] = df.apply(lambda row: int((row.date_to - row.date_from) / np.timedelta64(1,'D')), axis=1)
        return df.assign(length_of_stay=lambda row: ((row.date_to - row.date_from) / np.timedelta64(1,'D')).astype(int))
    ...
    def add_book_to_arrival(df):
        ...
        # Write your code here #
        ########################
        df['book_to_arrival'] = df.apply(lambda row: (row.date_from - row.booking_date) / np.timedelta64(1,'D'), axis=1)
        df = df.astype({'book_to_arrival': np.int64})
        return df
...
#         nr_of_day = pd.date_range(df.date_from, df.date_to, freq='D').to_series().dt.dayofweek
#         nr_of_day = nr_of_day.isin([4, 5])
#         print(nr_of_day)
#         print((nr_of_day == True).any())
        df['weekend_stay'] = df.apply(lambda row: str((pd.date_range(row.date_from, row.date_to, freq='D').to_series().dt.dayofweek.isin([5, 6]) == True).any()), axis=1)
        return df
    ...
    def add_night_price(df):
        ...
        # Write your code here #
        ########################
        df['night_price'] = df.apply(lambda x: (x.accommodation_price / x.length_of_stay) / x.n_rooms, axis=1).round(2)
        return df
    ...
    def aggregate_group_reservations(self, df):
        ...
        #merge
        merged_columns = pd.merge(sum_columns, mean_columns, on='group_id')
        merged_columns = pd.merge(merged_columns, mode_columns, on='group_id')
        merged_columns = pd.merge(merged_columns, first_columns, on='group_id')
        
        #concatenate
        df_concatenated = pd.concat([non_group_reservations, merged_columns])
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        # Write your code here #
        ########################
        group = df.groupby('room_group_id', as_index=False).agg({'night_price': 'mean'})
        df['room_segment'] = df.apply(lambda x: self.map_value_to_bucket(float(group.loc[group['room_group_id'] == x.room_group_id].night_price), self.room_segment_buckets), axis=1)
